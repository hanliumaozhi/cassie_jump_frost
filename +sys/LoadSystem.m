function [sys,domains,guards] = LoadSystem(robot, load_path, varargin)
    ...
    jumping = cassie.domain.double_support(robot, load_path, 'guard', 'left_lift');
...
%             sys = setEdgeProperties(sys, srcs, tars, 'Guard', {left_impact, right_impact});
            ...
            domains = [left_stance, right_stance];
            guards = [right_impact, left_impact];
            
            % Define hybrid system
            sys = HybridSystem('Cassie');
            sys = addVertex(sys, {'LeftStance', 'RightStance'}, 'Domain', {left_stance, right_stance});
            srcs = {'LeftStance', 'RightStance'};
            tars = {'RightStance', 'LeftStance'};
            sys = addEdge(sys, srcs, tars);
            sys = setEdgeProperties(sys, srcs, tars, 'Guard', {right_impact, left_impact});
